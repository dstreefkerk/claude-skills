...
.SYNOPSIS
    ...
    Same checks and error filters as Microsoft's CI workflow (.github/actions/entrypoint.ps1
    in the Azure-Sentinel repo).
...
.DESCRIPTION
    ...
    The ARM-TTK module is auto-installed on first run by cloning from GitHub
    (https://github.com/Azure/arm-ttk) into .arm-ttk/ at the current git repository root.

    Test skips and error filters match Microsoft's CI entrypoint.ps1 exactly:
    ...
    Source: derived from Azure-Sentinel/.script/local-validation/run-arm-ttk.ps1, which
    mirrors Microsoft's CI workflow logic. Copied into this skill for portability so the
    validation toolchain works without requiring an Azure-Sentinel repo checkout. Re-sync
    periodically when Microsoft updates their CI skip-list or filters.
...
.NOTES
    Requires PowerShell 7+ and git. On first run, takes ~30 seconds to clone arm-ttk.
    Subsequent runs use the cached clone (re-clone manually to update arm-ttk).
#>
